...
for (const app of ['codex','hermes','opencode']) for (const os of ['windows','macos','linux']) for (const arch of ['amd64','arm64']) {
 ...
 if(app==='hermes'){assert.equal(a.payload.catalog['gpt-5.5'].supports_vision,true);assert.ok(a.payload.plugin['__init__.py'].includes('class AIZaminImageGenProvider'))}
