...
- **Hermes:** creates two native profiles: `aizamin-lite` (constrained routes, minimal terminal agent) and `aizamin-standard` (main routes, full tools/vision/image). A native provider plugin loads authenticated live catalogs on startup, so later catalog changes need no installer rerun. Existing customers need one migration install. Obsolete installer-owned chat profiles are archived with conversations intact; personal/default profiles remain untouched. Both retain profile-scoped Whisper STT. Requires Hermes v0.21.3+. Start a new session with `hermes -p PROFILE`; `/model` does not switch profiles. See `configurer/README.md` for details and verification limits.
